...
// MongoDB Connection
const MONGODB_URI = process.env.MONGODB_URI || 'mongodb://admin:password123@mongodb_container:27017/taskmanager?authSource=admin';
...
async function migrateUser(user) {
  ...
  try {
    ...
    const rawUser = await mongoose.connection.db.collection('users').findOne({ userId: uid });
    if (rawUser && rawUser.tasks && typeof rawUser.tasks === 'object') {
      const ops = [];
      for (const [date, tasks] of Object.entries(rawUser.tasks)) {
        if (Array.isArray(tasks) && tasks.length > 0) {
          ops.push({ updateOne: { filter: { userId: uid, date }, update: { $setOnInsert: { userId: uid, date, tasks } }, upsert: true } });
        }
      }
    ...
    if (rawUser && rawUser.dailyData && typeof rawUser.dailyData === 'object') {
      ...
      for (const [date, dd] of Object.entries(rawUser.dailyData)) {
        if (dd && typeof dd === 'object') {
          ops.push({ updateOne: { filter: { userId: uid, date }, update: { $setOnInsert: { userId: uid, date, ...dd } }, upsert: true } });
        }
      }
      if (ops.length > 0) await DailyData.bulkWrite(ops);
    }
    // Migrate ideaDump
    if (rawUser && Array.isArray(rawUser.ideaDump) && rawUser.ideaDump.length > 0) {
      const ops = rawUser.ideaDump.map(idea => ({
        updateOne: { filter: { userId: uid, id: idea.id }, update: { $setOnInsert: { userId: uid, ...idea } }, upsert: true }
      }));
      await Idea.bulkWrite(ops);
    }
    // Migrate ideaTodos
    if (rawUser && Array.isArray(rawUser.ideaTodos) && rawUser.ideaTodos.length > 0) {
      const ops = rawUser.ideaTodos.map(todo => ({
        updateOne: { filter: { userId: uid, id: todo.id }, update: { $setOnInsert: { userId: uid, ...todo } }, upsert: true }
      }));
    ...
    if (rawUser && Array.isArray(rawUser.notes) && rawUser.notes.length > 0) {
      const ops = rawUser.notes.map(note => ({
        updateOne: { filter: { userId: uid, id: note.id }, update: { $setOnInsert: { userId: uid, ...note } }, upsert: true }
      }));
    ...
    await mongoose.connection.db.collection('users').updateOne(
      { userId: uid },
      { $set: { migrated: true }, $unset: { tasks: '', dailyData: '', ideaDump: '', ideaTodos: '', notes: '' } }
    );
    user.migrated = true;
...
const authLimiter = rateLimit({
  ...
  standardHeaders: true, legacyHeaders: false, skip: checkPinBypass,
  keyGenerator: (req) => {
    const uid = req.body && req.body.userId ? req.body.userId.trim().toLowerCase() : null;
...
function validateDateParam(req, res, next) {
  if (!isValidDate(req.params.date)) return res.status(400).json({ error: 'Invalid date format. Use YYYY-MM-DD.' });
  next();
}

function emptyTaskSlot() { return { text: '', description: '', status: 'Todo', carryForwardCount: 0 }; }

function emptyDailyData() {
  return { doNotDo: ['', '', ''], dailyReward: '', brainDump: '', antiToDo: [], reflectionWell: '', reflectionImprove: '' };
}
...
async function authenticate(req, res, next) {
  ...
  if (authHeader && authHeader.startsWith('Bearer ')) tokenStr = authHeader.split(' ')[1];
  else if (req.query.token) tokenStr = req.query.token;
  if (!tokenStr) return res.status(401).json({ error: 'Unauthorized' });
  try {
    const decoded = jwt.verify(tokenStr, JWT_SECRET);
    if (decoded.isAdmin) return res.status(401).json({ error: 'Admin tokens cannot access user routes' });
...
app.post('/api/register', registerLimiter, async (req, res) => {
  try {
    ...
    res.status(201).json({ token, userId: uid, preferences: newUser.preferences, northStarGoal: '' });
...
app.get('/api/tasks/:date', authenticate, validateDateParam, async (req, res) => {
  const doc = await TaskDay.findOne({ userId: req.userId, date: req.params.date });
  const tasks = doc ? doc.tasks : [emptyTaskSlot(), emptyTaskSlot(), emptyTaskSlot()];
  res.json({ tasks });
});

app.post('/api/tasks/:date', authenticate, validateDateParam, async (req, res) => {
  if (isDateInPast(req.params.date)) return res.status(403).json({ error: 'Cannot modify tasks for past dates.' });
  const MAX_TASKS = 5;
...
app.post('/api/carry-over', authenticate, async (req, res) => {
  ...
  const sourceDoc = await TaskDay.findOne({ userId: req.userId, date: sourceDate });
  ...
  const targetDoc = await TaskDay.findOne({ userId: req.userId, date: targetDate });
  let targetTasks = targetDoc ? targetDoc.tasks : [emptyTaskSlot(), emptyTaskSlot(), emptyTaskSlot()];

  if (unfinished.length === 0) return res.json({ carried: 0, tasks: targetTasks });
  ...
  if (unfinished.length > capacity) {
    return res.status(409).json({
      ...
      capacity, unfinishedCount: unfinished.length, tasks: targetTasks
    });
  }
  ...
  for (const task of unfinished) {
    ...
    else if (targetTasks.length < MAX_TASKS) targetTasks.push(carriedTask);
    else break;
    carried++;
  }
...
app.get('/api/carry-over-check/:date', authenticate, validateDateParam, async (req, res) => {
  const MAX_TASKS = 5;
  ...
  const sourceDoc = await TaskDay.findOne({ userId: req.userId, date: yesterday });
  const sourceTasks = sourceDoc ? sourceDoc.tasks : [];
  const unfinished = sourceTasks.filter(t => t.text && t.text.trim() !== '' && t.status !== 'Done' && t.status !== 'Cancelled');

  const targetDoc = await TaskDay.findOne({ userId: req.userId, date: targetDate });
  const targetTasks = targetDoc ? targetDoc.tasks : [emptyTaskSlot(), emptyTaskSlot(), emptyTaskSlot()];
  const emptySlots = targetTasks.filter(t => !t.text || t.text.trim() === '').length;
  const appendRoom = Math.max(0, MAX_TASKS - targetTasks.length);
  const capacity = emptySlots + appendRoom;
...
app.get('/api/daily-data/:date', authenticate, validateDateParam, async (req, res) => {
  const doc = await DailyData.findOne({ userId: req.userId, date: req.params.date });
  res.json(doc ? {
    doNotDo: doc.doNotDo, dailyReward: doc.dailyReward, brainDump: doc.brainDump,
    antiToDo: doc.antiToDo, reflectionWell: doc.reflectionWell, reflectionImprove: doc.reflectionImprove
  } : emptyDailyData());
});

app.post('/api/daily-data/:date', authenticate, validateDateParam, async (req, res) => {
  if (isDateInPast(req.params.date)) return res.status(403).json({ error: 'Cannot modify daily data for past dates.' });

  const updateFields = {};
  const body = req.body;
  if (body.doNotDo !== undefined) updateFields.doNotDo = body.doNotDo;
  if (body.dailyReward !== undefined) updateFields.dailyReward = body.dailyReward;
  ...
  if (body.reflectionWell !== undefined) updateFields.reflectionWell = body.reflectionWell;
  if (body.reflectionImprove !== undefined) updateFields.reflectionImprove = body.reflectionImprove;
...
app.get('/api/task-dates', authenticate, async (req, res) => {
  const taskDays = await TaskDay.find({ userId: req.userId }, { date: 1, tasks: 1, _id: 0 });
  const dateInfo = {};
...
app.post('/api/settings/password', authenticate, async (req, res) => {
  try {
    ...
    await req.user.save();
    res.json({ success: true });
  } catch (err) { console.error('Password change error:', err); res.status(500).json({ error: 'Server error' }); }
});
...
app.delete('/api/account', authenticate, async (req, res) => {
  ...
  } catch (err) { console.error('Account delete error:', err); res.status(500).json({ error: 'Server error' }); }
});
...
app.get('/api/ideas', authenticate, async (req, res) => {
  const ideas = await Idea.find({ userId: req.userId }, { _id: 0, userId: 0, __v: 0 }).sort({ createdAt: 1 });
  res.json({ ideas });
});
...
app.post('/api/ideas', authenticate, async (req, res) => {
  ...
  const idea = new Idea({
    userId: req.userId, id: genId(),
    title: title.trim().slice(0, MAX_TEXT_LENGTH),
    body: (body || '').trim().slice(0, MAX_DESC_LENGTH)
  });
  await idea.save();
  res.status(201).json({ success: true, idea: { id: idea.id, title: idea.title, body: idea.body, createdAt: idea.createdAt } });
});
...
app.put('/api/ideas/:id', authenticate, async (req, res) => {
  ...
  await idea.save();
  res.json({ success: true, idea: { id: idea.id, title: idea.title, body: idea.body, createdAt: idea.createdAt } });
});

app.delete('/api/ideas/:id', authenticate, async (req, res) => {
  const result = await Idea.deleteOne({ userId: req.userId, id: req.params.id });
...
app.get('/api/idea-todos', authenticate, async (req, res) => {
  const todos = await IdeaTodo.find({ userId: req.userId }, { _id: 0, userId: 0, __v: 0 }).sort({ createdAt: 1 });
...
app.post('/api/idea-todos', authenticate, async (req, res) => {
  ...
  await todo.save();
  res.status(201).json({ success: true, todo: { id: todo.id, title: todo.title, completed: todo.completed, createdAt: todo.createdAt } });
});

app.put('/api/idea-todos/:id', authenticate, async (req, res) => {
  const { title, completed } = req.body;
  const todo = await IdeaTodo.findOne({ userId: req.userId, id: req.params.id });
  ...
  await todo.save();
  res.json({ success: true, todo: { id: todo.id, title: todo.title, completed: todo.completed, createdAt: todo.createdAt } });
});

app.delete('/api/idea-todos/:id', authenticate, async (req, res) => {
  const result = await IdeaTodo.deleteOne({ userId: req.userId, id: req.params.id });
...
app.get('/api/notes', authenticate, async (req, res) => {
  const notes = await Note.find({ userId: req.userId }, { _id: 0, userId: 0, __v: 0 }).sort({ createdAt: 1 });
  res.json({ notes });
});
...
app.post('/api/notes', authenticate, async (req, res) => {
  ...
  await note.save();
  res.status(201).json({ success: true, note: { id: note.id, title: note.title, content: note.content, createdAt: note.createdAt, updatedAt: note.updatedAt } });
});
...
app.put('/api/notes/:id', authenticate, async (req, res) => {
  ...
  await note.save();
  res.json({ success: true, note: { id: note.id, title: note.title, content: note.content, createdAt: note.createdAt, updatedAt: note.updatedAt } });
});

app.delete('/api/notes/:id', authenticate, async (req, res) => {
  const result = await Note.deleteOne({ userId: req.userId, id: req.params.id });
...
app.post('/api/timer', authenticate, async (req, res) => {
  try {
    await User.updateOne({ userId: req.userId }, { $set: { timerState: req.body } });
    res.json({ success: true });
  } catch (err) {
...
app.get('/api/export', authenticate, async (req, res) => {
  ...
  const tasksObj = {}; taskDays.forEach(d => { tasksObj[d.date] = d.tasks; });
  const dailyObj = {}; dailyDays.forEach(d => {
    dailyObj[d.date] = { doNotDo: d.doNotDo, dailyReward: d.dailyReward, brainDump: d.brainDump, antiToDo: d.antiToDo, reflectionWell: d.reflectionWell, reflectionImprove: d.reflectionImprove };
  });

  const exportData = {
    userId: uid, tasks: tasksObj, dailyData: dailyObj,
    northStarGoal: req.user.northStarGoal, preferences: req.user.preferences,
    ideaDump: ideas, ideaTodos, notes, exportedAt: new Date().toISOString()
  };
  res.setHeader('Content-Disposition', `attachment; filename="taskmanager-${uid}-export.json"`);
  res.setHeader('Content-Type', 'application/json');
  res.json(exportData);
});

// ========== IMPORT ==========
app.post('/api/import', authenticate, upload.single('importFile'), async (req, res) => {
  try {
    let importData;
    if (req.file) {
      try { importData = JSON.parse(req.file.buffer.toString('utf8')); }
    ...
    for (const dateKey of dateKeys) {
      ...
      const sanitized = tasks.slice(0, 5).map(sanitizeTask);
      await TaskDay.findOneAndUpdate({ userId: uid, date: dateKey }, { $set: { tasks: sanitized } }, { upsert: true });
    }
    ...
    if (importData.dailyData && typeof importData.dailyData === 'object') {
      for (const dateKey of Object.keys(importData.dailyData).slice(0, MAX_IMPORT_DATES)) {
        ...
        await DailyData.findOneAndUpdate({ userId: uid, date: dateKey }, {
          $set: {
    ...
    if (typeof importData.northStarGoal === 'string') req.user.northStarGoal = importData.northStarGoal.slice(0, MAX_TEXT_LENGTH);
    ...
    if (Array.isArray(importData.ideaDump)) {
      for (const idea of importData.ideaDump) {
        ...
        await Idea.findOneAndUpdate(
          ...
          { $setOnInsert: { userId: uid, id: idea.id, title: idea.title || '', body: idea.body || '', createdAt: idea.createdAt || new Date() } },
          { upsert: true }
        );
      }
    }
    ...
    if (Array.isArray(importData.ideaTodos)) {
      for (const todo of importData.ideaTodos) {
        ...
        await IdeaTodo.findOneAndUpdate(
          ...
          { $setOnInsert: { userId: uid, id: todo.id, title: todo.title || '', completed: !!todo.completed, createdAt: todo.createdAt || new Date() } },
          { upsert: true }
        );
      }
    }
    ...
    if (Array.isArray(importData.notes)) {
      for (const note of importData.notes) {
        ...
        await Note.findOneAndUpdate(
          ...
          { $setOnInsert: { userId: uid, id: note.id, title: note.title || '', content: note.content || '', createdAt: note.createdAt || new Date(), updatedAt: note.updatedAt || new Date() } },
          { upsert: true }
        );
      }
    }
  ...
  } catch (err) { console.error('Import error:', err); res.status(500).json({ error: 'Server error' }); }
});
...
app.post('/api/admin/login', async (req, res) => {
  ...
  if (username !== ADMIN_USER || password !== ADMIN_PASS) return res.status(401).json({ error: 'Invalid admin credentials' });
  const token = jwt.sign({ isAdmin: true, username }, JWT_SECRET, { expiresIn: '4h' });
  res.json({ token, username });
});

app.get('/api/admin/users', authenticateAdmin, async (req, res) => {
  const users = await User.find({}, { passwordHash: 0, __v: 0 }).sort({ createdAt: -1 });
  ...
  for (const u of users) {
    ...
    result.push({
      userId: u.userId, disabled: !!u.disabled, migrated: !!u.migrated,
      hasPin: !!u.recoveryPin, createdAt: u.createdAt,
...
app.post('/api/admin/users', authenticateAdmin, async (req, res) => {
  ...
  } catch (err) { console.error('Admin create user error:', err); res.status(500).json({ error: 'Server error' }); }
});
...
app.post('/api/admin/users/:userId/toggle', authenticateAdmin, async (req, res) => {
  const user = await User.findOne({ userId: req.params.userId });
  if (!user) return res.status(404).json({ error: 'User not found' });
...
app.get('/api/admin/users/:userId/export', authenticateAdmin, async (req, res) => {
  const uid = req.params.userId;
  const user = await User.findOne({ userId: uid });
  if (!user) return res.status(404).json({ error: 'User not found' });
  ...
  const exportData = {
    userId: uid, tasks: tasksObj, dailyData: dailyObj,
    northStarGoal: user.northStarGoal, preferences: user.preferences,
    ideaDump: ideas, ideaTodos, notes, exportedAt: new Date().toISOString()
  };
  res.setHeader('Content-Disposition', `attachment; filename="taskmanager-${uid}-export.json"`);
  res.setHeader('Content-Type', 'application/json');
