...
function isPastDate() {
  return fmt(selectedDate) < fmt(new Date());
}
...
document.getElementById("login-form").addEventListener("submit", async (e) => {
  ...
  try {
    ...
    if (!res.ok) {
      ...
      if (res.status === 429) {
        document
          .getElementById("login-pin-container")
...
document
  ...
  .addEventListener("submit", async (e) => {
    ...
    if (pw !== document.getElementById("register-confirm").value) {
...
function loginSuccess(data) {
  ...
  localStorage.setItem("tm_userId", currentUserId);
...
function applyPreferences(prefs) {
  ...
  const accentTheme = isDark
    ? prefs.accentDark || "purple"
    : prefs.accentLight || "purple";
  ...
  if (accentTheme === "custom" && customHex) {
    if (typeof generateCustomStyle === "function")
  ...
  if (prefs.bgDark !== undefined) {
    document.documentElement.style.setProperty(
      "--bg-color-dark",
      `hsl(var(--ac-h), var(--ac-s), ${prefs.bgDark}%)`,
    );
  }
  if (prefs.bgLight !== undefined) {
    document.documentElement.style.setProperty(
      "--bg-color-light",
      `hsl(var(--ac-h), var(--ac-s), ${prefs.bgLight}%)`,
    );
  }
...
function _setupAccentColorSettings() {
  ...
  function hslaToHex(h, s, l, a) {
    ...
    const f = (n) => {
      const k = (n + h / 30) % 12;
      return l - a1 * Math.max(Math.min(k - 3, 9 - k, 1), -1);
    };
    return (
      "#" +
      [f(0), f(8), f(4), a]
  ...
  function openColorPickerModal() {
    ...
    const isDark = document.documentElement.classList.contains("dark");
    const currentHex =
      customTrigger?.dataset.currentHex ||
      (isDark ? currentPrefs.customHexDark : currentPrefs.customHexLight) ||
      "#3b82f6";

    const parsed = hexToHsla(currentHex);
    let hue = parsed.h,
      sat = parsed.s,
      light = parsed.l,
      alpha = parsed.a;
    ...
    document
      .getElementById("color-picker-cancel")
      .addEventListener("click", () => modal.remove());
    document
      .getElementById("color-picker-close")
      .addEventListener("click", () => modal.remove());
    ...
    document
      .getElementById("color-picker-apply")
...
function _setupGradientSettings() {
  ...
  if (gradBtn && gradModal) {
    ...
    const openGradModal = () => {
      ...
      originalOp =
        currentPrefs.auraOp !== undefined ? currentPrefs.auraOp : 0.15;
      originalSat =
        currentPrefs.auraSat !== undefined ? currentPrefs.auraSat : 100;
      originalBlur =
        currentPrefs.auraBlur !== undefined ? currentPrefs.auraBlur : 60;
      ...
      if (isDark) {
        ...
        gradBgSlider.max = 20;
        originalBg =
          currentPrefs.bgDark !== undefined ? currentPrefs.bgDark : 2;
      } else {
        gradBgSlider.min = 80;
        gradBgSlider.max = 100;
        originalBg =
          currentPrefs.bgLight !== undefined ? currentPrefs.bgLight : 98;
      }
    ...
    const closeGradModal = () => {
      ...
      if (currentPrefs.bgDark !== undefined) {
        document.documentElement.style.setProperty(
          "--bg-color-dark",
          `hsl(var(--ac-h), var(--ac-s), ${currentPrefs.bgDark}%)`,
        );
      } else {
      ...
      if (currentPrefs.bgLight !== undefined) {
        document.documentElement.style.setProperty(
          "--bg-color-light",
          `hsl(var(--ac-h), var(--ac-s), ${currentPrefs.bgLight}%)`,
        );
      } else {
...
function updateNorthStarDisplay() {
  ...
  el.textContent =
    northStarGoal && northStarGoal.trim()
      ? `"${northStarGoal}"`
      : "Set your North Star in Settings →";
}
...
function updateDateDisplay() {
  ...
  document.getElementById("selected-date-display").textContent =
    selectedDate.toLocaleDateString("en-US", opts);
  const isToday = fmt(selectedDate) === fmt(new Date());
  document.getElementById("selected-date-label").textContent = isToday
    ? "Today's Focus"
    : "Viewing past date";
...
async function loadTasks(ds) {
  try {
    ...
    while (currentTasks.length < DEFAULT_TASKS)
      currentTasks.push({
        text: "",
        description: "",
        status: "Todo",
...
function renderTasks() {
  ...
  const filledCount = currentTasks.filter(
    (t) => t.text && t.text.trim(),
  ).length;
  const displayMax = Math.max(
    DEFAULT_TASKS,
    Math.min(currentTasks.length, MAX_TASKS),
  );
  document.getElementById("task-count-label").textContent =
    `${filledCount} / ${displayMax}`;
  ...
  container
    ...
    .forEach((el) => el.addEventListener("input", onTaskChange));
  container
    .querySelectorAll(".task-desc-area")
    .forEach((el) => el.addEventListener("input", onTaskChange));
  ...
  container.querySelectorAll(".status-btn").forEach((btn) => {
    btn.addEventListener("click", (e) => {
      e.stopPropagation();
      openStatusDropdown(parseInt(btn.dataset.idx), btn);
    });
  });
...
function onTaskChange(e) {
  ...
  if (e.target.classList.contains("task-input")) {
    const oldText = currentTasks[i].text || "";
    const newText = e.target.value;
    currentTasks[i].text = newText;

    // Reset carry-over badge if the text is cleared OR completely replaced
    const isCleared = newText.trim() === "";
    const isReplaced =
      oldText.length > 2 &&
      newText.length > 0 &&
    ...
    if (isCleared || isReplaced) {
      currentTasks[i].carryForwardCount = 0;
      const wrapper = e.target.closest(".task-input-wrapper");
      if (wrapper) {
        const badge = wrapper.querySelector(".carry-badge");
        if (badge) badge.remove();
      }
    }
  } else if (e.target.classList.contains("task-desc-area"))
    currentTasks[i].description = e.target.value;
...
async function saveTasks() {
  try {
    await fetch(`/api/tasks/${fmt(selectedDate)}`, {
      method: "POST",
      headers: headers(),
      body: JSON.stringify({ tasks: currentTasks }),
    });
    showSaved("tasks-saved");
    loadTaskDates();
  } catch (e) {
...
function renderDailyData() {
  ...
  const past = isPastDate();
  document.querySelectorAll(".donotdo-input").forEach((el, i) => {
    el.value = dnds[i] || "";
    el.disabled = past;
  });
  const rewardEl = document.getElementById("reward-input");
  rewardEl.value = currentDailyData.dailyReward || "";
  rewardEl.disabled = past;
  const brainEl = document.getElementById("braindump-textarea");
  brainEl.value = currentDailyData.brainDump || "";
  brainEl.disabled = past;
  document.getElementById("reflection-well").value =
    currentDailyData.reflectionWell || "";
  document.getElementById("reflection-improve").value =
    currentDailyData.reflectionImprove || "";
...
document.querySelectorAll(".donotdo-input").forEach((el) => {
  el.addEventListener("input", () => {
    ...
    debounce("donotdo", () =>
      saveDailyField({ doNotDo: currentDailyData.doNotDo }, "donotdo-saved"),
    );
  });
});
...
document.getElementById("braindump-textarea").addEventListener("input", () => {
  ...
  debounce("braindump", async () => {
    ...
    setTimeout(() => {
      document.getElementById("braindump-status").textContent = "";
    }, 1500);
  });
});
...
document.getElementById("reflection-well").addEventListener("input", () => {
  currentDailyData.reflectionWell =
    document.getElementById("reflection-well").value;
  debounce("reflectionWell", () =>
    saveDailyField(
      { reflectionWell: currentDailyData.reflectionWell },
      "reflection-saved",
    ),
  );
});
document.getElementById("reflection-improve").addEventListener("input", () => {
  currentDailyData.reflectionImprove =
    document.getElementById("reflection-improve").value;
  debounce("reflectionImprove", () =>
    saveDailyField(
      { reflectionImprove: currentDailyData.reflectionImprove },
      "reflection-saved",
    ),
  );
});
...
async function saveDailyField(fields, savedId) {
  try {
    await fetch(`/api/daily-data/${fmt(selectedDate)}`, {
      method: "POST",
      headers: headers(),
    ...
    console.error("saveDailyField:", e);
  }
}
...
function renderAntiToDo() {
  ...
  list.querySelectorAll(".antitodo-remove").forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      renderAntiToDo();
      saveDailyField({ antiToDo: currentDailyData.antiToDo });
    });
  });
}

document.getElementById("antitodo-add").addEventListener("click", addAntiToDo);
document.getElementById("antitodo-input").addEventListener("keydown", (e) => {
  if (e.key === "Enter") {
    e.preventDefault();
    addAntiToDo();
  }
...
function addAntiToDo() {
  ...
  currentDailyData.antiToDo.push(val);
  input.value = "";
  renderAntiToDo();
  saveDailyField({ antiToDo: currentDailyData.antiToDo });
}
...
function checkReflectionUnlock() {
  ...
  if (unlocked) {
    ...
    wellTA.disabled = !!past;
    improveTA.disabled = !!past;
    if (past) {
    ...
    wellTA.disabled = true;
    improveTA.disabled = true;
...
function buildCalendar() {
  ...
  monthSel.innerHTML = months
    .map(
      (m, i) =>
        `<option value="${i}" ${i === calendarDate.getMonth() ? "selected" : ""}>${m}</option>`,
    )
    .join("");
  yearSel.innerHTML = "";
  const cy = new Date().getFullYear();
  for (let y = cy - 3; y <= cy + 2; y++)
    yearSel.innerHTML += `<option value="${y}" ${y === calendarDate.getFullYear() ? "selected" : ""}>${y}</option>`;
  renderCalendarGrid();
}
...
async function checkCarryOver() {
  ...
  try {
    const res = await fetch(`/api/carry-over-check/${todayStr}`, {
      headers: headers(),
    });
    const data = await res.json();
    const banner = document.getElementById("carry-over-banner");
    if (data.hasUnfinished) {
      // Check if user has already dismissed this specific carry-over notification
      const dismissedKey = `dismissed_carryover_${todayStr}_${data.sourceDate}`;
      if (localStorage.getItem(dismissedKey) === "true") {
        return;
      }

      document.getElementById("carry-over-detail").textContent =
        `${data.count} task(s) from ${data.sourceDate} • Capacity: ${data.capacity} slot(s) free`;
      banner.classList.remove("hidden");
      banner._sourceDate = data.sourceDate;
    ...
    console.error("checkCarryOver:", e);
  }
}

document
  .getElementById("carry-over-accept")
  .addEventListener("click", async () => {
    const banner = document.getElementById("carry-over-banner");
    const todayStr = fmt(new Date());
    try {
      const res = await fetch("/api/carry-over", {
        method: "POST",
        headers: headers(),
        body: JSON.stringify({
          sourceDate: banner._sourceDate,
          targetDate: fmt(selectedDate),
        }),
      });
      const data = await res.json();
      if (res.status === 409) {
        ...
        showToast(
          data.message ||
            "Capacity Reached: Not enough room to carry over tasks.",
          "error",
        );
        return;
      }
      ...
      localStorage.setItem(
        `dismissed_carryover_${todayStr}_${banner._sourceDate}`,
        "true",
      );
      showToast(
        `✓ Carried over ${data.carried} task(s) successfully`,
        "success",
      );
    } catch (e) {
...
document
  .getElementById("idea-modal-close")
...
function renderIdeaTodos() {
  ...
  visibleTodos.forEach((todo) => {
    ...
    delBtn.className =
      "w-6 h-6 flex items-center justify-center rounded border border-transparent text-gray-400 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 transition-all opacity-0";
    ...
    checkBtn.addEventListener("click", () =>
      toggleIdeaTodo(todo.id, !todo.completed),
    );
...
document
  .getElementById("idea-todo-filter-btn")
  ?.addEventListener("click", () => {
...
async function toggleIdeaTodo(id, completed) {
  try {
    ...
    console.error("toggleIdeaTodo:", e);
  }
}
...
async function deleteIdeaTodo(id) {
  try {
    ...
    console.error("deleteIdeaTodo:", e);
  }
}
...
function startEditIdeaTodo(todo, titleSpan) {
  ...
  input.type = "text";
  input.className =
    "flex-1 text-sm font-medium px-2 py-1 rounded bg-gray-100 dark:bg-gray-800 border-none outline-none focus:ring-1 focus:ring-accent-500 -ml-2";
...
document.getElementById("user-badge").addEventListener("click", () => {
  ...
  document.getElementById("north-star-input").value = northStarGoal || "";
});
...
document
  ...
  .addEventListener("click", async () => {
    ...
    const accentTheme = isDark
      ? currentPrefs.accentDark || "purple"
      : currentPrefs.accentLight || "purple";
    document.documentElement.setAttribute("data-theme", accentTheme);
    const customHex = isDark
      ? currentPrefs.customHexDark
      : currentPrefs.customHexLight;
    if (
      accentTheme === "custom" &&
      customHex &&
      typeof generateCustomStyle === "function"
    ) {
      generateCustomStyle(customHex);
    }
...
document
  .getElementById("north-star-save")
  .addEventListener("click", async () => {
    northStarGoal = document.getElementById("north-star-input").value.trim();
    updateNorthStarDisplay();
    try {
      await fetch("/api/settings/north-star", {
        method: "POST",
        headers: headers(),
        body: JSON.stringify({ northStarGoal }),
      });
    } catch {}
  });
...
importInput.addEventListener("change", async (e) => {
  ...
  const errEl = document.getElementById("data-msg");
  errEl.className = "text-sm text-center mt-2 hidden";
  try {
    ...
    if (!res.ok) {
      errEl.textContent = data.error;
      errEl.classList.remove("hidden", "text-green-500");
      errEl.classList.add("text-rose-500");
    } else {
      errEl.textContent = "Import successful! Reloading...";
      errEl.classList.remove("hidden", "text-rose-500");
      errEl.classList.add("text-green-500");
...
async function loadNotes() {
  try {
    ...
    if (currentNotes.length > 0 && !activeNoteId) {
      ...
      activeNoteId &&
      currentNotes.find((n) => n.id === activeNoteId)
    ) {
...
const saveNoteChanges = async () => {
  if (!activeNoteId) return;
  const title =
    document.getElementById("note-title-input").value.trim() || "Untitled Note";
...
document
  ...
  .addEventListener("input", () => debounce("saveNote", saveNoteChanges, 600));
document
  .getElementById("note-content-input")
  .addEventListener("input", () => debounce("saveNote", saveNoteChanges, 600));
...
function saveTimerState() {
  ...
  if (token) {
    fetch("/api/timer", {
      method: "POST",
      headers: headers(),
      body: JSON.stringify(timerState === "idle" ? {} : stateObj),
    }).catch(() => {});
  }
}
...
function scheduleAlarmAtEnd() {
  ...
  try {
    ...
    console.error("scheduleAlarmAtEnd failed:", e);
  }
}

function cancelScheduledAlarm() {
  scheduledAlarmNodes.forEach((osc) => {
    try {
      osc.stop();
    } catch {}
  });
...
function triggerTimerAlarm() {
  ...
  banner.className =
    "fixed top-4 left-1/2 transform -translate-x-1/2 z-[300] bg-gradient-to-r from-accent-600 to-accent-600 text-white px-6 py-4 rounded-2xl shadow-2xl flex items-center gap-4 max-w-sm";
  banner.innerHTML = `
  ...
  document
    .getElementById("timer-alarm-close")
    .addEventListener("click", stopTimerAlarm);
  setTimeout(() => {
    if (document.getElementById("timer-alarm-banner"))
      document.getElementById("timer-alarm-banner").remove();
  }, 31000);
}
...
function logout() {
  ...
  localStorage.removeItem("tm_userId");
  ...
  if (token && currentUserId) {
    try {
      ...
      if (res.ok) {
        const data = await res.json();
        applyPreferences(data.preferences || {});
        const nsRes = await fetch("/api/settings/north-star", {
          headers: headers(),
        });
        if (nsRes.ok) {
          const nsData = await nsRes.json();
          northStarGoal = nsData.northStarGoal || "";
        }
        showDashboard();
